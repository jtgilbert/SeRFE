...
class Dpred:
    ...
    def __init__(self, network, grain_size, reach_ids, width_table):
        ...
        self.mean_prop_mid = np.mean(np.array(self.Qc_mid)/np.array(self.Q2))
        self.mean_prop_low = np.mean(np.array(self.Qc_low)/np.array(self.Q2))
        self.mean_prop_high = np.mean(np.array(self.Qc_high)/np.array(self.Q2))
